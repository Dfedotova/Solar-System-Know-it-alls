...
public class WordsGrid : MonoBehaviour
{
    ...
    private void SetSquaresPosition()
    {
        ...
        var offsetForLevel = 0.0f;
        if (level == 1)
            offsetForLevel = 7.5f;
        if (level == 2)
            offsetForLevel = 9.5f; 
        if (level == 3)
            offsetForLevel = 11.5f;
    ...
    private Vector2 GetFirstSquarePosition()
    {
        ...
        var midWidthPosition = (((currentGameData.selectedBoardData.columns - 1) * squareSize.x) / 2) * 0.01f / 7.5f;
        var midWidthHeight = (((currentGameData.selectedBoardData.rows - 1) * squareSize.y) / 2) * 0.01f / 7.5f;

        startPosition.x = (midWidthPosition != 0) ? midWidthPosition * -1 : midWidthPosition;
        startPosition.y += midWidthHeight - 4;

        return startPosition / 4.5f;
    }
        ...
        if (currentGameData != null)
        {
            ...
            foreach (var squares in currentGameData.selectedBoardData.board)
            {
                foreach (var squareLetter in squares.row)
                {
                    ...
                    {
                        Debug.LogError("All fields in your array should have some letter!");
